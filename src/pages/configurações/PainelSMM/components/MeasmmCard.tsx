...
export const MeasmmCard = () => {
  ...
  const loadApiKey = async () => {
    try {
      if (window.electron?.utils) {
        const result = await window.electron.utils.getSmmApiKey('measmm')
  ...
  const handleOpenPanel = async () => {
    try {
      if (window.electron?.utils) {
        await window.electron.utils.openExternalUrl('https://measmm.com/')
      } else {
        window.open('https://measmm.com/', '_blank', 'noopener,noreferrer')
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (window.electron?.utils) {
        const result = await window.electron.utils.saveSmmApiKey('measmm', apiKey.trim())
        if (result.success) {
          setIsConfigured(true)
  ...
  const handleRemoveConfig = async () => {
    ...
    try {
      if (window.electron?.utils) {
        const result = await window.electron.utils.saveSmmApiKey('measmm', '')
        if (result.success) {
          setApiKey('')
  ...
  return (
    <div className="relative bg-gray-800/40 backdrop-blur-md rounded-lg border border-gray-600/30 shadow-lg p-6 hover:shadow-xl transition-shadow">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-semibold text-gray-100">Measmm</h3>
      </div>
