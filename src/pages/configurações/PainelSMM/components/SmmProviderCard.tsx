...
export const SmmProviderCard = () => {
  ...
  const loadApiKey = async () => {
    try {
      if (window.electron?.utils) {
        const result = await window.electron.utils.getSmmApiKey('smmprovider')
  ...
  const handleOpenPanel = async () => {
    try {
      if (window.electron?.utils) {
        await window.electron.utils.openExternalUrl('https://smmprovider.co/')
      } else {
        window.open('https://smmprovider.co/', '_blank', 'noopener,noreferrer')
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (window.electron?.utils) {
        const result = await window.electron.utils.saveSmmApiKey('smmprovider', apiKey.trim())
        if (result.success) {
          setIsConfigured(true)
  ...
  const handleTestConnection = async () => {
    ...
    try {
      // Simular teste de conexão (aqui você pode implementar uma chamada real à API)
  ...
  const handleRemoveConfig = async () => {
    ...
    try {
      if (window.electron?.utils) {
        // Remover a API Key salvando uma string vazia
        const result = await window.electron.utils.saveSmmApiKey('smmprovider', '')
        if (result.success) {
          setApiKey('')
  ...
  return (
    <div className="relative bg-gray-800/40 backdrop-blur-md rounded-lg border border-gray-600/30 shadow-lg p-6 hover:shadow-xl transition-shadow">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-semibold text-gray-100">SmmProvider</h3>
      </div>
